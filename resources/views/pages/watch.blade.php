                  ...
                  <div id="halim-list-server">
                     ...
                     <ul class="nav nav-tabs" role="tablist">
                        @foreach($server as $key => $ser)
                           <li role="presentation" class=" server-1"><a href="#server-0" aria-controls="server-0" role="tab" data-toggle="tab"><i class="hl-server"></i>{{$ser->title}}</a></li>
                        @endforeach
                     </ul>
                        ...
                        <div role="tabpanel" class="tab-pane active server-1" id="server-0">
                           <div class="halim-server">
                              <ul class="halim-list-eps ">
                                 @foreach($server as $key => $ser)
                                    @foreach($server_movie as $key => $ser_mov)
                                       @if($ser_mov->server == $ser->id)
                                          <!----server phim---->
                                          <li class="halim-episode"><span class="halim-btn halim-btn-2  halim-info-1-1 box-shadow">{{$ser->title}}
                                             
                                          </span></li>
                                          <!----tap phim---->
                                          <ul class="halim-list-eps">
                                            @foreach($server_movie_list->sortBy('episode') as $key => $epi)
                                                @if($epi->server == $ser->id)
                                                <a href="{{url('xem-phim/'.$movie->slug.'/tap-'.$epi->episode.'/server-'.$epi->server)}}">
                                                   <li class="halim-episode">
                                                      <span class="halim-btn halim-btn-2 {{$tapphim==$epi->episode && $server_active=='server-'.$ser->id ? 'active' : ''}} halim-info-1-1 box-shadow"  data-title="Xem Phim {{$movie->title}} - Tập {{$epi->episode}} - {{$movie->name_eng}} - vietsub _ Thuyết Minh " data-h1="{{$movie->title}} - Tập {{$epi->episode}}">{{$epi->episode}}
